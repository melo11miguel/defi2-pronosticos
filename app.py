...
# =========================================================
# DESCARGA DE DATOS (ROBUSTA PARA CUALQUIER TICKER)
# =========================================================
...
# =========================================================
# MODELO 1: GBM (CORREGIDO Y ESTABLE)
# =========================================================
def backtest_gbm(prices_train, prices_test, n_paths=1000, seed=42):

    train_arr = prices_train.astype(float).values
    test_arr = prices_test.astype(float).values

    # Log-returns
    log_ret = np.log(train_arr[1:] / train_arr[:-1])
    mu = np.mean(log_ret) * 252
    sigma = np.std(log_ret) * np.sqrt(252)
    dt = 1 / 252
    ...
    rmse = compute_rmse(test_arr, preds)

    return preds, rmse, {"mu": mu, "sigma": sigma}


# =========================================================
# MODELO 2: Heston (SIMPLIFICADO)
# =========================================================
...
test_size = st.slider("Proporción de datos para test (backtesting)", 0.10, 0.50, 0.20)
